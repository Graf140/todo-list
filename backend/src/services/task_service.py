...
class TaskService:
    ...
    def get_tasks(
        ...
        # enrich tasks with comments and relations
        for task in tasks:
            task["comments"] = CommentRepository.get_by_task_id(task["task_id"], 3)
            for c in task["comments"]:
                c["user_name"] = f"{c['first_name']} {c['last_name']}"
                del c["first_name"]
                del c["last_name"]
    ...
    def get_task(task_id: int, current_user: dict) -> dict:
        ...
        # enrich
        task["comments"] = CommentRepository.get_by_task_id(task_id)
        for c in task["comments"]:
            c["user_name"] = f"{c['first_name']} {c['last_name']}"
            del c["first_name"]
            del c["last_name"]
